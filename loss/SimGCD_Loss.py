...
class SupConLoss(torch.nn.Module):
    ...
    def forward(self, features, labels=None, mask=None):
        """Compute loss for backbone. If both `labels` and `mask` are None,
...
class DistillLoss_WoWarm(nn.Module):
    def __init__(self, ncrops=2, teacher_temp=0.04, student_temp=0.1):
        super().__init__()
        self.student_temp = student_temp
        self.ncrops = ncrops
