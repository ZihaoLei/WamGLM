...
class BalSCL(nn.Module):
    def __init__(self, cls_num_list=None, temperature=0.08):
    ...
    def forward(self, centers1, features, targets, ):
        ...
        per_ins_weight = torch.tensor([batch_cls_count[i] for i in targets], device=device).view(1, -1).expand(2 * batch_size, 2 * batch_size + len(self.cls_num_list)) - mask
