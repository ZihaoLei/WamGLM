...
from lora_mixin import replace_linear_with_lora
from loss.BCLloss import BalSCL
...
class FineTuneVisualGLMModel(VisualGLMModel):
    def __init__(self, args, transformer=None, **kw_args):
        ...
        if args.use_lora:
            self.add_mixin("lora",
                           LoraMixin(args.num_layers, args.chatglm_lora_rank, layer_range=args.chatglm_layer_range),
                           reinit=True)
            self.get_mixin("eva").model.glm_proj = replace_linear_with_lora(self.get_mixin("eva").model.glm_proj, 1,
                                                                            args.chatglm_lora_rank)
            self.get_mixin("eva").model.vit.add_mixin("lora", LoraMixin(args.num_layers, args.vit_lora_rank,
                                                                        layer_range=args.vit_layer_range), reinit=True)

        elif args.use_qlora:
            self.add_mixin("lora", LoraMixin(args.num_layers, args.lora_rank, layer_range=args.layer_range, qlora=True),
                           reinit=True)
        self.args = args
    ...
    def disable_untrainable_params(self):
        ...
        for n, p in self.named_parameters():
            flag = False
            for e in enable:
                if (e.lower() in n.lower() and 'vit' not in n.lower()) or (e.lower() in n.lower() and 'vit' in n.lower() and '38' in n.lower()):
                    flag = True
                    break
            if not flag:
...
def forward_step_adaptation(data_iterator, model, args, timers):
    ...
    augmentation_1 = transforms.Compose([transforms.RandomRotation(degrees=(-90, 90)),
                                         transforms.RandomGrayscale(p=0.3)
                                         ])
    aug_images_1 = augmentation_1(images)
    aug_images_1 = aug_images_1.to("cuda")

    augmentation_2 = transforms.Compose([transforms.RandomHorizontalFlip(),
                                         transforms.RandomVerticalFlip(),
                                         transforms.RandomApply([
                                             transforms.ColorJitter(0.4, 0.4, 0.4, 0.1)  # not strengthened
                                         ], p=0.8)
                                         ])
    aug_images_2 = augmentation_2(images)
    aug_images_2 = aug_images_2.to("cuda")  ##添加

    augmentation_3 = transforms.Compose([transforms.RandomHorizontalFlip(),
                                         transforms.RandomVerticalFlip()
                                         ])

    aug_images_3 = augmentation_3(images)
    aug_images_3 = aug_images_3.to("cuda")  ##添加

    timers('batch generator').stop()

    batch_size = labels.shape[0]

    inputs = torch.cat([aug_images_1, aug_images_2, aug_images_3], dim=0)
    logits, feat_mlp, centers = model.get_mixin("eva").model.vit(inputs)[0]
    centers = centers[:38]
    ...
    # loss_scl = InfoNCELoss()
    cls_numlist = [800, 800, 800, 800, 800, 800, 119, 800, 692, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800,
                   800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800, 800]
    # print(len(cls_numlist))
    loss_scl = BalSCL(cls_num_list=cls_numlist)
    loss_ce = LogitAdjust(cls_num_list=cls_numlist)
    loss = loss_ce(logits, labels) + loss_scl(centers, features, labels)*0.35
    # l1 = loss_ce(logits, labels)
    # l2 = loss_scl(centers, features, labels)
    # print(l1.cpu().item(), l2.cpu().item())
...
class FewShotDataset(Dataset):
    def __init__(self, path, processor, tokenizer, args):
        ...
        for item in tqdm(data):
            image_path = item['images'][0]  # 图像路径
            image = processor(Image.open(image_path).convert('RGB'))
            ...
            for i, message in enumerate(messages):
                if message['role'] == 'user':
                    ...
                    if prompt.startswith('<image>'):
                        prompt = prompt.replace('<image>', '')  # 去除<image>标签
                    if i == 0:
                        prompt = context + "问：" + prompt + "\n答："
                    else:
                        prompt = context + "\n问：" + prompt + "\n答："
                    input0 = tokenizer.encode("<img>", add_special_tokens=False)
                    input1 = [tokenizer.pad_token_id] * args.image_length
                    input2 = tokenizer.encode("</img>" + prompt, add_special_tokens=False)
                    a_ids = sum([input0, input1, input2], [])
                    aid_lengths.append(len(a_ids))
                    if len(a_ids) > args.max_source_length - 1:
                        print(
                            f'warning: prompt max_source_length exceeded: {len(a_ids)} ; truncating to {args.max_source_length - 1} tokens.')
                    ...
                    bid_lengths.append(len(b_ids))

                    if len(b_ids) > args.max_target_length - 2:
                        print(
                            f'warning: target max_source_length exceeded: {len(b_ids)} ; truncating to {args.max_target_length - 2} tokens.')
        ...
        # 绘制 aid_lengths 的直方图
        plt.hist(aid_lengths, bins=10, alpha=0.5, label='aid_lengths')

        # 绘制 bid_lengths 的直方图
        plt.hist(bid_lengths, bins=10, alpha=0.5, label='bid_lengths')

        # 添加标题和标签
        plt.title('Histogram of aid_lengths and bid_lengths')
        plt.xlabel('Length')
...
if __name__ == '__main__':
    ...
    if train_vit:
        ...
        training_main(args, model_cls=model, forward_step_function=forward_step_adaptation,
                      create_dataset_function=create_adaptation_dataset_function, collate_fn=data_collator, forward_step_eval=forward_step_eval)
    ...
    if train_chat:
        ...
        # args.train_iters = 15000
        # args.save_interval = 15000
        # args.eval_interval = 2500
