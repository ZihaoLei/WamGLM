...
class ClsMixin(BaseMixin):
    def __init__(self, hidden_size, num_classes):
        ...
        self.projector = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.BatchNorm1d(hidden_size), nn.ReLU(inplace=True), nn.Linear(hidden_size, 256))
        self.head_fc = nn.Sequential(nn.Linear(hidden_size, hidden_size), nn.BatchNorm1d(hidden_size), nn.ReLU(inplace=True), nn.Linear(hidden_size, 256))

    def final_forward(self, logits, **kw_args):
        representation = logits[:, 0]
        representation_proj = F.normalize(self.projector(representation), dim=1)
        logits = self.classifier(representation)
        centers_logits = F.normalize(self.head_fc(self.classifier.weight), dim=1)
        return logits, representation_proj, centers_logits
...
class EVAViT(ViTModel):
    def __init__(self, args, transformer=None, **kwargs):
        ...
        self.add_mixin("cls", ClsMixin(1408, 38))
    ...
    def start_adaptation(self, args):
        self.del_mixin("cls")
        self.add_mixin("cls", ClsMixin(args['hidden_size'], 38))
