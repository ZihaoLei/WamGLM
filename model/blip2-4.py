...
from model.projector import DINOHead
...
class ClsMixin(BaseMixin):
    def __init__(self, hidden_size):
        super().__init__()
        self.projector = DINOHead(in_dim=hidden_size, out_dim=36, nlayers=3)

    def final_forward(self, logits, **kw_args):
        representation = logits[:, 0]
        pred, x_proj = self.projector(representation)
        return pred, x_proj
...
class EVAViT(ViTModel):
    ...
    def start_adaptation(self, args):
        self.del_mixin("cls")
        self.add_mixin("cls", ClsMixin(args['hidden_size']))
